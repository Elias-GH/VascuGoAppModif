    ...
    public boolean onPreferenceClick(final Preference preference) {
        ...
        switch (preference.getTitleRes()) {
            case R.string.goal:
                ...
                String[] numberValues= new String[np.getMaxValue()/np.getMinValue()];
                int step = 500;
                ...
                builder.setPositiveButton(android.R.string.ok, new OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        np.clearFocus();
                        prefs.edit().putInt("goal", np.getValue()*500).commit();
                        preference.setSummary(getString(R.string.goal_summary, np.getValue()*500));
                        dialog.dismiss();
